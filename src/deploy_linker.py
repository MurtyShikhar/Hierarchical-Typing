...
def get_tensors_from_minibatch(minibatch, num_iter, config, entity_hierarchy_linked_list, train ):
    ...
    if train:
        all_tensors['gold_ids'], _ = pad_sequences(minibatch.data['gold_ids'], 0, torch.FloatTensor, config.gpu)
...
def train(model, train, dev, test, config, entity_hierarchy_linked_list, entity_hierarchy_linked_list_orig):
    ...
    for epoch in xrange(config.num_epochs):
        ...
        for minibatch in train.get_batch():
            ...
            linking_loss   = loss_criterion_multilabel(scores['linking_logits'], all_tensors['gold_ids']) # treat linking as a multi-label loss with ancestors
...
if __name__ == "__main__":
    ...
    train_data = LinkingDataset(train_lines, ATTRIBS, entity_dict, vocab_dict, type_dict, alias_table, pretrained_embeddings, config_obj.batch_size,UMLS_transformer, entity_hierarchy_linked_list, True, config_obj.encoder)
    dev_data   = LinkingDataset(dev_lines, ATTRIBS, entity_dict,   vocab_dict, type_dict, alias_table, pretrained_embeddings, config_obj.batch_size,UMLS_transformer, entity_hierarchy_linked_list, False,config_obj.encoder)
    test_data  = LinkingDataset(test_lines, ATTRIBS, entity_dict,  vocab_dict, type_dict, alias_table, pretrained_embeddings, config_obj.batch_size,UMLS_transformer, entity_hierarchy_linked_list, False,config_obj.encoder)
